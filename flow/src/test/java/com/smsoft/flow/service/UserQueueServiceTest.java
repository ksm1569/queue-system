...
import com.smsoft.flow.EmbeddedRedis;
import com.smsoft.flow.exception.ApplicationException;
...
class UserQueueServiceTest {
    ...
    void registerWaitQueue() {
        ...
        StepVerifier.create(userQueueService.registerWaitQueue("default", 106L))
                .expectNext(2L)
                .verifyComplete();

        StepVerifier.create(userQueueService.registerWaitQueue("default", 108L))
                .expectNext(3L)
                .verifyComplete();
    }
    ...
    void allowUserAfterRegisterWaitQueue() {
        StepVerifier.create(userQueueService.registerWaitQueue("default", 100L)
                        ...
                        .then(userQueueService.registerWaitQueue("default", 107L))
                )
                .expectNext(1L)
                .verifyComplete();
    }
    ...
    void isNotAllowedByToken() {
        StepVerifier.create(
                        userQueueService.isAllowedByToken("default", 100L, "")
                )
                .expectNext(false)
                .verifyComplete();
    }
    ...
    void isAllowedByToken() {
        StepVerifier.create(
                        userQueueService.isAllowedByToken("default", 100L, "d333a5d4eb24f3f5cdd767d79b8c01aad3cd73d3537c70dec430455d37afe4b8")
                )
                .expectNext(true)
                .verifyComplete();
    }
    ...
    void generateToken() {
        StepVerifier.create(
                        userQueueService.generateToken("default", 100L)
                )
